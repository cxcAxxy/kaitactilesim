#!/usr/bin/env python3
"""Serial poker-draw pi0.5+T-Rex evaluation with deterministic review selection."""
...
RUNNER = ROOT / "scripts/workcell/run_poker_pi05_trex.py"
...
def fingerprints(openpi_root: Path) -> dict:
  paths = [
    RUNNER,
    ...
    openpi_root / "src/openpi/tactile_expert/model.py",
    openpi_root / "src/openpi/models_pytorch/pi0_pytorch.py",
    openpi_root / "src/openpi/models_pytorch/preprocessing_pytorch.py",
    openpi_root / "src/openpi/shared/image_tools.py",
  ]
  result = {
...
def main() -> None:
  ...
  parser.add_argument("--tactile-refine-every", type=int, default=5)
  ...
  if args.execute_steps <= 0 or args.tactile_refine_every <= 0:
    parser.error("execute-steps and tactile-refine-every must be positive")
  ...
  protocol = {
    ...
    "tactile_refine_offsets": list(range(0, args.execute_steps, args.tactile_refine_every)),
    "control_hz": 30,
  ...
  for trial_index, seed in enumerate(args.seeds):
    if fingerprints(openpi_root) != frozen:
    ...
    command = [
      ...
      "--deployment-manifest",
      str(deployment_path),
      "--openpi-root", str(openpi_root),
      "--base-pytorch", str(args.base_pytorch.expanduser().resolve()),
      "--expert-checkpoint", str(args.expert_checkpoint.expanduser().resolve()),
      "--tactile-root", str(args.tactile_root.expanduser().resolve()),
      "--tactile-refine-every", str(args.tactile_refine_every),
      "--seed",
      str(seed),
      "--xy-jitter-mm",
    ...
    row = {
      ...
      "tactile_refinements": summary.get("stats", {}).get("tactile_refinements", 0),
      "tactile_updates": summary.get("stats", {}).get("tactile_updates", 0),
