#!/usr/bin/env python3
"""Serial USB EgoSteer evaluation with deterministic review-video selection."""
...
RUNNER = ROOT / "scripts/workcell/run_usb_egosteer_policy.py"
REVIEW_FILES = (
  "review.mp4",
  "review.json",
  "frames.jsonl",
...
def load_deployment_manifest(path):
  ...
  if payload["task"] != "usb-insert":
    raise RuntimeError(f"expected usb-insert deployment, got {payload['task']!r}")
...
def fingerprints():
  ...
  files = [
    ...
    source / "tasks/usb_insert/config.py",
    source / "tasks/usb_insert/setup.py",
    source / "tasks/usb_insert/task.py",
    source / "tasks/usb_insert/review_metrics.py",
    source / "shared/simulation.py",
    source / "shared/egosteer_adapter.py",
    source / "shared/egosteer_client.py",
    source / "shared/rendering.py",
    source / "shared/evaluation_video.py",
  ]
  result = {
...
def main():
  ...
  if args.xy_jitter_mm < 0 or args.yaw_jitter_deg < 0:
    parser.error("randomization ranges must be nonnegative")
  ...
  for trial_index, seed in enumerate(args.seeds):
    ...
    aggregate = {
      ...
      "termination_counts": dict(
        Counter(result["termination"] for result in results)
      ),
      "failure_stage_counts": dict(
        Counter(
          result["evaluation"].get("failure_stage")
          for result in results
          if result["valid_trial"] and not result["success"]
        )
      ),
    ...
    if not valid:
      raise RuntimeError(
        f"seed {seed}: infrastructure/unclassified failure; batch paused"
      )
